...
class Permutation(Transform):
    ...
    def __call__(self, data, labels, tlabels):
        ...
        transformed_data, new_labels, new_tlabels, transforms = self.define_containers(tlabels)
...
class CyclicTranslation2D(Transform):
    ...
    def get_samples(self, dim_v, dim_h):
        ...
        if self.sample_method == "linspace":
#             n_v_transforms = int(self.fraction_transforms * dim_v)
#             n_h_transforms = int(self.fraction_transforms * dim_h)
#             all_transforms = list(
#                 itertools.product(
#                     np.linspace(0, dim_v - 1, n_v_transforms),
#                     np.linspace(0, dim_h - 1, n_h_transforms),
#                 )
#             )
...
#                 xt = translate2d(x, tv, th)
                xt = np.roll(x, (int(tv), int(th)), axis=(0, 1))
                transformed_data.append(xt)
